(function () {
    ...
    function myPlugin() {
        Lampa.Listener.follow('full', function (e) {
            if (e.type == 'complite') {
                var btn = $('<div class="full-start__button selector view--btn" style="margin-top: 10px;">Мій Сервер</div>');

                btn.on('hover:enter hover:click hover:touch', function () {
                    Lampa.Loading.start();

                    var network = new Lampa.Reguest();
                    // Формуємо запит до твого сервера
                    ...
                    network.silent(api, function (json) {
                        ...
                        Lampa.Loading.stop();
                        Lampa.Noty.show('Помилка мережі: ' + network.errorDecode(a, c));
                    });
                });
